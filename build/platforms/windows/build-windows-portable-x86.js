/**
 * A script that allow to build app for Windows portable x86
 *
 * @requires config
 * @requires client
 * @requires folders
...
async function build() {
	...
	const releaseFolderName = `${config.windowsConfig.npmPortable86Path} v${config.version}`;
	...
	try {
		...
		await rename.renameItem(config.windowsConfig.npmPortable86Path, releaseFolderName);
		console.log('Finish!');
