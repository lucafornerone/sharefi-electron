/**
 * A script that allow to build app for macOS installer x64
 *
 * @requires config
 * @requires client
 * @requires folders
...
async function build() {
	...
	try {
		...
		if (!await npm_run.buildScript(config.macOsConfig.npmInstaller64Script)) {
			return;
		}
		console.log('Finish!');
		...
		await rename.renameItem(config.macOsConfig.npmInstaller64File, `${config.macOsConfig.npmPortable64Path} v${config.version}.dmg`);
		console.log('Finish!');
