...
async function build() {
	...
	try {		
		...
		if (!await npm_run.buildScript(config.linuxConfig.npmInstallerRedHatx86_64Script)) {
			return;
		}
		console.log('Finish!'); 
		...
		await rename.renameItem(`${config.completeName}-${config.version}-1.x86_64.rpm`, `${config.appName}-x86_64-v${config.version}.rpm`);
		console.log('Finish!');
